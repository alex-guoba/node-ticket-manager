...
(function() {
  var EMPTY_OBJ, MIN_FIELD_SELECTION, STATUS, Schema, TicketSchema, _, mongoose, paginator, schemaOptions, schemaStructure, timestamps;
  ...
  TicketSchema.statics.arrangeAssignment = function(options, callback) {
    ...
    if (!worker) {
      return callback(new Error("missing request params, worker:" + worker));
    }
    if (options.id != null) {
      query = {
        _id: options.id
      ...
      if (!category) {
        return callback(new Error("missing request params: category"));
      }
      query = {
        $and: [
          {
